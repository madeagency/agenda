...
Agenda.prototype.every = function(interval, names, data, options, cb) {
  ...
  if (typeof names === 'string' || names instanceof String) {
    var genericJobName = this.getGenericJobDefinition(names);
    if (genericJobName) {
      return createGenericJob(interval, genericJobName, data, options, cb);  
    } else {
  ...
  function createGenericJob(interval, name, data, options, cb) {
...
Agenda.prototype.schedule = function(when, names, data, cb) {
  ...
  if (typeof names === 'string' || names instanceof String) {
    var genericJobName = this.getGenericJobDefinition(names);
    if (genericJobName) {
      return createGenericJob(when, genericJobName, data, cb);  
    } else {
  ...
  function createGenericJob(when, name, data, cb) {
    var job = self.create(name, data);
    job.schedule(when);
    job.save(cb);
    return job;
...
Agenda.prototype.getGenericJobDefinition = function(jobName) {
  var definitions = this._definitions;
  // get all jobs defined with regex
  var genericJobs = Object.keys(definitions).filter(function(key) {
    return key.match(/\/*\//) // check if key starts and ends with / -- regex
  });

  var matches = genericJobs.filter(function(name) {
    var regex = name.substring(1, name.length-1);
    return jobName.match(new RegExp(regex));
  });
